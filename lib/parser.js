...
Parser.prototype.parseAST = function(context, parent) {
	...
	while ( context.i < length ) {
		...
		if ( char === this.opts.VAR_CLOSE ) {
			if ( ! parent )
				throw unexpected(char, context.i);
			break;

		} else if ( char === this.opts.VAR_OPEN || (special_hash && char === this.opts.PLURAL_ARG) ) {
...
Parser.prototype.parseText = function(context, parent, include_separator = true, include_space = true) {
	...
	while ( context.i < length ) {
		...
		if ( char === this.opts.VAR_OPEN ||
				char === this.opts.VAR_CLOSE ||
				(!include_separator && char === this.opts.VAR_SEP) ||
				(!include_space && SPACE.test(char)) ||
				(special_hash && char === this.opts.PLURAL_ARG) )
			break;
		...
		if ( char === this.opts.ESCAPE ) {
			...
			if ( next === this.opts.ESCAPE ) {
				...
				out += next;
				context.i++;
			} else if ( next === this.opts.VAR_OPEN || next === this.opts.VAR_CLOSE || (special_hash && next === this.opts.PLURAL_ARG) || ! include_space || ! include_separator ) {
				// Special Character
				out += next;
				while( ++context.i < length ) {
					next = msg[context.i];
...
Parser.prototype.parseElement = function(context, parent) {
	...
	if ( special_hash && char === this.opts.PLURAL_ARG ) {
		context.i++;
		return {v: parent.v}
	}

	if ( char !== this.opts.VAR_OPEN )
		throw expected(this.opts.VAR_OPEN, char, context.i);
	...
	} else if ( char !== this.opts.VAR_SEP )
		throw expected(`${this.opts.VAR_SEP} or ${this.opts.VAR_CLOSE}`, char, context.i);
	...
	if ( this.opts.submessage_types.includes(out.t) ) {
		...
		const format = this.parseText(context, null, true, false);
		if ( ! format )
	...
	char = msg[context.i];
	if ( char !== this.opts.VAR_CLOSE )
		throw expected(this.opts.VAR_CLOSE, char, context.i);
...
Parser.prototype.parseSubmessage = function(context, parent) {
	...
	if ( msg[context.i] !== this.opts.VAR_OPEN )
		throw expected(this.opts.VAR_OPEN, context);

	context.i++;
	const out = this.parseAST(context, parent);

	if ( msg[context.i] !== this.opts.VAR_CLOSE )
		throw expected(this.opts.VAR_CLOSE, context);
