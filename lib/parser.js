...
proto.parseAST = function(context, parent) {
	const msg = context.msg,
		opts = this.opts,
		length = msg.length,
		special_hash = parent && opts.subnumeric_types.includes(parent.t),
		out = [];
...
proto.parseText = function(context, parent, include_separator = true, include_space = true) {
	const msg = context.msg,
		opts = this.opts,
		length = msg.length,
		special_hash = parent && opts.subnumeric_types.includes(parent.t);
	...
	while ( context.i < length ) {
		...
		if ( char === opts.ESCAPE ) {
			...
			if ( next === opts.ESCAPE ) {
				...
				out += next;
				context.i++;
			} else if ( next === opts.OPEN || next === opts.CLOSE || (special_hash && next === opts.SUB_VAR) || ! include_space || ! include_separator ) {
				// Special Character
				out += next;
				while( ++context.i < length ) {
					next = msg[context.i];
...
proto.parseElement = function(context, parent) {
	const msg = context.msg,
		opts = this.opts,
		special_hash = parent && opts.subnumeric_types.includes(parent.t);
	...
	if ( char !== opts.OPEN )
		throw expected(opts.OPEN, char, context.i);
	...
	char = msg[context.i];
	if ( char === opts.CLOSE ) {
		if ( opts.submessage_types.includes(out.t) )
	...
	// Formatting
	if ( opts.subnumeric_types.includes(out.t) ) {
...
proto.parseSubmessages = function(context, parent) {
	const msg = context.msg,
		length = msg.length,
		out = {};
...
proto.parseSubmessage = function(context, parent) {
	const msg = context.msg,
		opts = this.opts;
...
proto.parseOffset = function(context) {
	const msg = context.msg,
