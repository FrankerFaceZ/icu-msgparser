...
class Parser {
	...
	parseAST(context, parent) {
		const msg = context.msg,
			length = msg.length,
			special_hash =
				parent && this.opts.subnumeric_types.includes(parent.t),
			out = [];
		...
		while (context.i < length) {
			...
			if (char === this.opts.CLOSE) {
				if (!parent) throw unexpected(char, context.i);
				break;
			} else if (
				char === this.opts.OPEN ||
			...
			if (context.i === start) throw unexpected(char, context.i);
		}

		return out;
	}
	
	parseText(context, parent, include_separator = true, include_space = true) {
		const msg = context.msg,
			length = msg.length,
			special_hash =
				parent && this.opts.subnumeric_types.includes(parent.t);
		...
		while (context.i < length) {
			...
			if (char === this.opts.ESCAPE) {
				...
				if (next === this.opts.ESCAPE) {
					...
					next === this.opts.OPEN ||
					next === this.opts.CLOSE ||
					(special_hash && next === this.opts.SUB_VAR) ||
					!include_space ||
					!include_separator
				) {
					// Special Character
	...
	parseElement(context, parent) {
		...
		if (!id) throw expected('placeholder id', char, context.i);
		...
		if (char === this.opts.CLOSE) {
			...
			throw expected(
				`${this.opts.SEP} or ${this.opts.CLOSE}`,
				char,
				context.i
			);
		...
		if (!type) throw expected('type', context);
		...
		if (this.opts.submessage_types.includes(out.t)) {
			...
			if (!format) throw expected('format', context);
	...
	parseSubmessages(context, parent) {
		const msg = context.msg,
			length = msg.length,
			out = {};
		...
		while (context.i < length) {
			const char = msg[context.i];
			if (char === this.opts.CLOSE) break;

			const selector = this.parseText(context, null, true, false);
			if (!selector) throw expected('sub-message selector', context);
	...
	parseSubmessage(context, parent) {
		const msg = context.msg;
	...
	parseOffset(context) {
		...
		if (msg.slice(context.i, context.i + OFFSET.length) !== OFFSET) return;

		context.i += OFFSET.length;
		skipSpace(context);

		const start = context.i;
		while (context.i < length && /\d/.test(msg[context.i])) context.i++;

		if (start === context.i) throw expected('number', context);
...
function expected(char, found, index) {
	...
	return new SyntaxError(
		`expected ${char} at position ${index} but found ${found || 'eof'}`
	);
}
...
function skipSpace(context) {
	...
	while (context.i < length && SPACE.test(msg[context.i])) context.i++;
}

exports = module.exports = new Proxy(Parser, {
